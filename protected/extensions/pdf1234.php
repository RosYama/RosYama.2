...
class pdf1234{
	...
	protected function text_manyholes($models){
		...
		$ar['holes'][] = $model->type->name.', '.$model->ADDRESS.($model->COMMENT1 ? ', '.trim($model->COMMENT1) : '').($model->description_locality ? ', '.trim($model->description_locality) : '').($model->description_size ? ', '.trim($model->description_size) : '').", долгота: $model->LONGITUDE, широта: $model->LATITUDE, http://rosyama.ru".CController::createUrl("/holes/view",Array('id'=>$model->ID))."";    
		}
