...
class pdf1234{
	...
	protected function getTypeTemplate(){
		$type=$this->temp;
		$model=$this->models[0];
		$ar['body0'] = '    '.$this->params['date1.day'].'.'.$this->params['date1.month'].'.'.$this->params['date1.year'].' мною на территории дороги по адресу: '.$this->params['street'].($model->description_locality && $this->requestForm && $this->requestForm->showDescriptions ? ', '.$model->description_locality : '')."";
			$regex = "#{descr}(.*?){/descr}#s";
